...
if [ "$AWS_EXECUTION_ENV" != "CloudShell" ]; then kind --version; else echo "skipping because it's not installed with CloudShell";fi 
errorcheck kind
logger "green" "checking octant..."
if [ "$AWS_EXECUTION_ENV" != "CloudShell" ]; then octant version; else echo "skipping because it's not installed with CloudShell";fi 
errorcheck octant 
logger "green" "checking code-server..."
if [ "$AWS_EXECUTION_ENV" != "CloudShell" ]; then code-server --version; else echo "skipping because it's not installed with CloudShell";fi
